...
namespace BlackCat\Database\Packages\SyncErrors\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\SyncErrors\Definitions;
use BlackCat\Database\Packages\SyncErrors\Criteria;
use BlackCat\Database\Packages\SyncErrors\Dto\SyncErrorDto as Dto;
use BlackCat\Database\Packages\SyncErrors\Mapper\SyncErrorDtoMapper as RowMapper;
...
final class SyncErrorRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\SyncErrors\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\SyncErrors\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\SyncErrors\Dto\SyncErrorDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\SyncErrors\Dto\SyncErrorDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\SyncErrors\Mapper\SyncErrorDtoMapper::fromRow($row);
    }
