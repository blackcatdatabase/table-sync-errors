...
namespace BlackCat\Database\Packages\SyncErrors\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\SyncErrors\Dto\SyncErrorDto;
use BlackCat\Database\Packages\SyncErrors\Mapper\SyncErrorDtoMapper;
 ...
 */
final class SyncErrorsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private SyncErrorRepository $syncErrorRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'sync_errors' => $this->syncErrorRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\SyncErrors\ModuleException("Record {$idStr} not found for locking.");
            }
