...
namespace BlackCat\Database\Packages\SyncErrors\Joins;
...
final class SyncErrorsJoins {
    ...
    /**
     * FK: sync_errors -> peer_nodes
     * LEFT JOIN vw_peer_nodes AS $as ON $as.id = $alias.peer_id
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinPeerNodes(string $alias = 't', string $as = 'j0'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_peer_nodes AS ' . $as . ' ON ' . $as . '.id = ' . $alias . '.peer_id' . ' ', []];
    }
